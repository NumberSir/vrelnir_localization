...
class ReleaseBuild:
    ...
    def generate_release_note_discord(self):
        with open(FILE_README, "r", encoding="utf-8") as fp:
            lines = fp.readlines()
        ...
        for line in lines:
            ...
            if 'issue-dc' in line:
                id_, url = re.findall(r"^\[issue-dc/(\d+)]: (.*?)$", line)[0]
                result = result.replace(f"[@issue-dc/{id_}][issue-dc/{id_}]", url)
            else:
                id_, url = re.findall(r"^\[issue/(\d+)]: (.*?)$", line)[0]
                result = result.replace(f"[issue/{id_}]", f"({url})")

        with open(DIR_BUILD / "discord.md", "w", encoding="utf-8") as fp:
...
async def main():
    ...
    async with httpx.AsyncClient(verify=False) as client:
        with Github(auth=Auth.Token(ACCESS_TOKEN)) as github:
            ...
            """ 发布 """  # TODO
            process.generate_release_note_discord()
