...
class ProjectDOL:
    ...
    async def update_dicts(self):
        ...
        old_version = Paratranz().get_current_version()
        tasks = [
            self._update_for_gather(old_file, new_file, json_file, old_version)
    ...
    async def _update_for_gather(self, old_file: Path, new_file: Path, json_file: Path, old_version: str):
        """
        gather 用
        ...
        if not new_file.exists():   # 旧文件在新版本中删除/改名了
            unavailable_file = DIR_RAW_DICTS / self._type / self._version / "csv" / "game" / "移除文件" / old_version / Path().joinpath(*old_file.parts[old_file.parts.index("utf8")+1:])
        ...
        unavailable_file = DIR_RAW_DICTS / self._type / self._version / "csv" / "game" / "失效词条" / old_version / Path().joinpath(*old_file.parts[old_file.parts.index("utf8")+1:]) if unavailables else None
